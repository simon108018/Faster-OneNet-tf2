...
def get_map(MINOVERLAP, draw_plot, path='./map_out'):
    ...
    if draw_plot:
        ...
        output_path = RESULTS_FILES_PATH + "/mAP_iou{:2d}.png".format(int(MINOVERLAP*100))
        to_show = True
        plot_color = 'royalblue'
        draw_plot_func(
            ap_dictionary,
...
def get_coco_map(class_names, path):
    ...
    result = os.path.join(COCO_PATH, 'result.txt')
    ...
    cocoEval.accumulate()
    cocoEval.summarize()
    l = np.array(['mAP','AP50','AP75','APs','APm','APl','AR1','AR10','AR100','ARs','ARm','ARl'])
    l = np.concatenate((l,cocoEval.stats)).reshape((2,-1))
    with open(result, 'w') as g:
        for i in range(l.shape[1]):
            g.write('{:5s}:{:.3f}\n'.format(str(l[0,i]),float(l[1,i])))

def get_coco_test_json(class_names, path):
    import numpy as np
    from pycocotools.coco import COCO
    from pycocotools.cocoeval import COCOeval
